...
const motions = {
    ...
    [Motions.WordJumpForward]: ({ editor: { state, view: { dispatch }, storage: { vimirror: vimirrorStorage }, }, }) => {
        ...
        doc.descendants((node, pos) => {
            if (!node.isBlock || nodeWithPos.node)
                return;
        ...
        if (!content)
            throw new Error("If theres no content, where the hell are you pressing the W from?");
        const inlineSelectionIndex = from - nodeWithPos.pos;
        let foundSeparator = false;
        let indexToJump = undefined;
    ...
    [Motions.WordJumpBackward]: ({ editor: { state, view: { dispatch }, storage: { vimirror: vimirrorStorage }, }, }) => {
        ...
        const content = nodeWithPos.node?.textContent;
        if (!content)
            throw new Error('If theres no content, where the hell are you pressing the "b" from?');
        const inlineSelectionIndex = from - nodeWithPos.pos;
        let foundSeparator = false;
        let indexToJump = undefined;
...
const Vimirror = Extension.create({
    name: "vimirror",
    addOptions() {
        return {
    ...
    addProseMirrorPlugins() {
        ...
        //   }
        // })
        const handleKey = ({ keys, type, motion, mode, action, }) => {
            const storage = getStorage();
